...
if __name__ == '__main__':
    ...
    double_compressed = double_jpeg_compression.detect('..//static//images//' + im_str)

    if(double_compressed): print('\nDouble compression detected')
    else: print('\nSingle compressed')

    print('\nRunning CFA artifact detection...\n')
    identical_regions_cfa = copy_move_cfa.detect('..//static//images//' + im_str, opt, args)
    print('\n' + str(identical_regions_cfa), 'CFA artifacts detected')

    print('\nRunning noise variance inconsistency detection...')
    noise_forgery = noise_variance.detect('..//static//images//' + im_str)
    ...
    if 'output' not in os.listdir('../'):
        mkdir('../output')
    copy_move_detection.detect('../static/images/', im_str, '../output/', blockSize=32)
